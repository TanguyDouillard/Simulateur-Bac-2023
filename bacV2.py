...
def trouver_notes_Tanguy():
    global Prenom
    saisie_Philo.delete(0, END)
    saisie_GO.delete(0, END)
    Prenom="Tanguy"
    l=[Prenom]
    cursor.execute("SELECT note_philosophie FROM eleves WHERE prenom=?",l)
    note_philo = cursor.fetchall()
    if note_philo== None :
        saisie_Philo.delete(0,END)
    else:
        saisie_Philo.insert(0,note_philo)
    cursor.execute("SELECT note_grand_oral FROM eleves WHERE prenom=?",l)
    note_GO = cursor.fetchall()
    if note_GO== None :
        saisie_GO.delete(0,END)
    else:
        saisie_GO.insert(0,note_GO)
...
def mention():
    ...
    GO = var_GO.get()
    l = [GO, philo,Prenom]
    cursor.execute("UPDATE eleves SET note_grand_oral = ?, note_philosophie = ? WHERE prenom=?", l)
